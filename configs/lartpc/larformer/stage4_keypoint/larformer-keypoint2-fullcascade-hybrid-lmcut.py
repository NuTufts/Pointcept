"""HYBRID chain + TRAINING-PARITY lm>=0.15 cut (2026-08-16):
OLD LoRA deghoster @ tau=0.2 + NEW m2frecipe slicer + NEW stage-3 segmenter
+ OLD attempt-2 keypoint model. See
larformer-fullcascade-hybrid-loradeghost-tau020.py for the rationale.
...
# training-parity LArMatch cut: the new slicer/stage-3 trained on a cache
# built from lm>=0.15-culled clouds; apply the same cut at inference
# (requires REAL lm scores in the input h5s, i.e. the *_lmscored copies)
_test["lm_score_val_threshold"] = 0.15
...
nu_thresh = 0.3
save_path = "exp/larformer_keypoint2_fullcascade_hybrid_lmcut_infer"
