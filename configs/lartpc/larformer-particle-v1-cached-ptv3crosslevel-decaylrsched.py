...
Model / data / loss are UNCHANGED; the only experimental deltas are:
  ...
  - Resumes from the flat-LR run's mid-epoch checkpoint
    (`RESUME_CHECKPOINT`, epoch=5 iter=18200/22588, global optimizer
    step 131140) with `CheckpointLoader(extend_scheduler=True)`: the
    old FlatWithDecayLR state is discarded, the new `DelayedCosineLR`
    is fast-forwarded to the checkpoint's true step count (requires
    the iter_in_epoch-aware fast-forward in hooks/misc.py), and the
    cosine decay begins exactly at the resume point.
  - `base_lr = 5e-5` (the LR the run is currently at — this becomes
    the cosine's start value via the new scheduler's `initial_lr`).
  - NO `reset_optimizer` — Adam moments carry over (the optimizer
    reset in `resume2B_resetoptim` was shown to be a no-op for the
    loss instability; see the stability doc §1.5).
  - Rationale: the previous runs' FlatWithDecayLR plateau decay never
    fired (its `step_epoch` driver hook `LREpochScheduler` was not in
    the hooks list), so the LR sat flat at the gradient-noise floor.
    See the stability doc §1.4.

Original config description follows.
--------------------------------------------------------------------
...
# =============================================================================
# Resume point for the schedule swap (R1)
# =============================================================================
# Mid-epoch checkpoint from the flat-5e-5 run (`..._resume2`), saved by
# IterCheckpointSaver with:
#   epoch=5  iter=18200/22588  ->  global optimizer step 131140
# (`IterCheckpointSaver` stores epoch = the IN-PROGRESS epoch, and
# iter_in_epoch = the next dataloader iter to run.)
#
# ITERS_PER_EPOCH is the actual len(train_loader) of this dataset/batch
# combo (22588), NOT the 410k/16 = 25625 estimate quoted in the old
...
RESUME_EPOCH         = 5
RESUME_ITER_IN_EPOCH = 18200
RESUME_STEP          = RESUME_EPOCH * ITERS_PER_EPOCH + RESUME_ITER_IN_EPOCH
...
model = dict(
    ...
    loss_kwargs=dict(
        ...
        weight_dn_loss=1.0,
        # R2 (stability doc §6): log-only per-batch diagnostics, computed
        # under no_grad on the final layer — never contribute to the loss.
        # Keys land as train_batch/loss_diag_* (and val/loss_diag_* via the
        # evaluator). Headliners: diag_mask_bce_rand / diag_dice_rand are
        # the STATIONARY-sampler controls for the mask loss (pure random
        # negatives); diag_mask_iou_matched is the train-time twin of
        # val/mask_iou. If loss_mask_primary rises while these hold/improve,
        # the rise is the adaptive-sampler artifact, not regression.
...
hooks = [
    # extend_scheduler=True: on resume, do NOT load the old scheduler's
    # state — build DelayedCosineLR from this config and fast-forward its
    # step counter to epoch*len(train_loader)+iter_in_epoch. The new
    # scheduler's initial_lr (= base_lr below) is preserved against the
    # optimizer-state load. Safe to leave on for subsequent auto-resumes:
    # DelayedCosineLR's LR is a pure function of the step counter, so the
    # recomputed fast-forward always lands on the right LR.
    # NOTE: no reset_optimizer — keep the Adam moments (stability doc §1.5).
...
# Training loop knobs
# =============================================================================
weight = RESUME_CHECKPOINT
resume = True
save_path        = "exp/larformer_particle_v1_cached_ptv3crosslevel_smallbatch_resume3_cosinedecay"
epoch            = 20
eval_epoch       = 20
...
# Optimizer / scheduler
# =============================================================================
# base_lr MUST equal the LR the resumed run is currently at: the new
# scheduler's initial_lr is taken from this, and the cosine starts here.
base_lr = 5.0e-5
param_dicts = None
