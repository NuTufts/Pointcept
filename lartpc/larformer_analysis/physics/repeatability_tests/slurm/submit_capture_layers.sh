#!/bin/bash
#
# submit_capture_layers.sh — Tier-B per-stage capture (capture_deghost_layers.py) to localize divergence
...
#SBATCH --job-name=sp_layers
#SBATCH --partition=gpu
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem-per-cpu=8000
#SBATCH --time=2:00:00
#SBATCH --output=/cluster/tufts/wongjiradlabnu/twongj01/pointcept_env/pointcept/lartpc/larformer_analysis/physics/repeatability_tests/slurm/logs/layers/cap.%j.log
#SBATCH --error=/cluster/tufts/wongjiradlabnu/twongj01/pointcept_env/pointcept/lartpc/larformer_analysis/physics/repeatability_tests/slurm/logs/layers/cap.%j.err
...
SP=${POINTCEPT}/lartpc/larformer_analysis/physics/single_photon
...
DETERM=${DETERM:-1}
mkdir -p ${CAPTURE_DIR} ${REPEAT}/slurm/logs/layers
...
[ -f \${REPREFIXED} ] || python3 ${POINTCEPT}/lartpc/data_prep/uboone_official/prefix_particle_ckpt.py \
...
python3 ${REPEAT}/capture_deghost_layers.py \
    ...
    --out-dir ${CAPTURE_DIR} \
    ${DET_FLAG} ${FP64_FLAG} ${TARGET_FLAG} ${MAX_FLAG} --device cuda
"
