...
def ratios(ntuple, cascade_dir, eq2, scale, min_pe=50.0):
...
def main():
    ...
    mc = ratios(args.mc_ntuple, args.mc_cascade, args.eq2, 1.0)
    d0 = ratios(args.data_ntuple, args.data_cascade, args.eq2, 1.0)
    ds = ratios(args.data_ntuple, args.data_cascade, args.eq2, args.data_scale)
    print(f">>> reco-NC {vlab} pred/obs: MC med {np.median(mc):.2f} (N={len(mc)})"
    ...
    ax.set(xlabel="predicted / observed PE (live PMTs)", ylabel="normalized",
           title=f"reco-NC {vlab} pi0: nu-slice pred/obs PE, MC vs bnb5e19\n"
                 "(run-aware dead mask; data with run1 gamma correction)")
    ax.legend(fontsize=8.5)
    ax.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(f"{args.plots}/predobs_mc_vs_data_{vlab}.png", dpi=110)
    plt.close(fig)
