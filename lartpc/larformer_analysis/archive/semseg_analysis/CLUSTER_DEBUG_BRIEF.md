...
| `lartpc/larformer_analysis/archive/semseg_analysis/run_semseg_inference.py` | Standalone inference driver written during the laptop debug session; faithfully reproduces the `LArTPCDataset` val pipeline |
| `lartpc/larformer_analysis/archive/semseg_analysis/histograms.py` | Accumulator schema (confusion + score histograms per origin) |
| `lartpc/larformer_analysis/archive/semseg_analysis/merge_shards.py`, `plot_metrics.py` | Shard merge + metric plotting |
...
./run_in_container.sh python lartpc/larformer_analysis/archive/semseg_analysis/run_semseg_inference.py \
...
./run_in_container.sh python lartpc/larformer_analysis/archive/semseg_analysis/plot_metrics.py \
...
The script `run_semseg_inference.py` was designed to mirror `lartpc/larformer_analysis/archive/deghost_analysis/run_deghost_inference.py` (which the user reports faithfully reproduces wandb val numbers for the deghoster model). Same H5-read approach, same per-shard histogram accumulator pattern, same Compose/Collect pipeline.
...
1. The user has a fully-functional deghost-analysis pipeline at `lartpc/larformer_analysis/archive/deghost_analysis/` (binary deghosting model evaluation). The new semseg pipeline at `lartpc/larformer_analysis/archive/semseg_analysis/` mirrors its structure.
