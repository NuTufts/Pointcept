...
The validation and test file list is on the Tufts cluster at:
  ...
  /cluster/tufts/wongjiradlabnu/twongj01/pointcept_env/pointcept/lartpc/data_prep/training_data/h5lists/h5list_mcall_lantern_valtest.txt


### Flash Info Maker

We already have the flash info maker script. It is located at `lartpc/data_prep/training_data/run_step5_flashinfo_pointcept_wconfig.sh`. 
...
2. From a Tufts head node:
   ```bash
   sbatch lartpc/larformer_analysis/slicer_eval/slurm/submit_valtest.sh \
          lartpc/larformer_analysis/slicer_eval/slurm/<your>.conf
   ```
...
| `view_event_flash.py` | Interactive Dash app: open one `perevent_*.h5` and overlay four per-PMT PE histograms on the same axis — observed in-time flash, selected predicted slice (× γ), GT-nu prediction (× γ), and the min-χ² slice under the current γ. γ slider + OOB-threshold dropdown + log-y toggle let you sweep flash-match conditions live. A per-PMT χ² contribution bar chart and a top-N slice table (selected highlighted blue, min-χ² highlighted orange) sit underneath. Use this to drill into why a specific event's χ² looks bad. Default `--port 8052`. |
...
```
twongj01@pax001:/cluster/tufts/wongjiradlabnu/twongj01/pointcept_env/pointcept$ python lartpc/larformer_analysis/slicer_eval/aggregate_metrics.py --perevent-dir exp/larformer_slicer_v1_cascaded_crosslevelrefiner_mixedq_maskdn/valtest_iter_24183/analysis/bnb_nu_pi0filter_corsika/ --output-dir exp/larformer_slicer_v1_cascaded_crosslevelrefiner_mixedq_maskdn/valtest_iter_24183/ --gamma-beam 225.0 --gamma-cosmic 225.0 --f-sys 0.1 
...
```
twongj01@pax001:/cluster/tufts/wongjiradlabnu/twongj01/pointcept_env/pointcept$ python lartpc/larformer_analysis/slicer_eval/aggregate_metrics.py --perevent-dir exp/larformer_slicer_v1_cascaded_ptv3hybrid_crosslevel_nonzeroinit_maskdn_noamp/valtest_iter_56600/analysis/bnb_nu_pi0filter_corsika/ --output-dir exp/larformer_slicer_v1_cascaded_ptv3hybrid_crosslevel_nonzeroinit_maskdn_noamp/valtest_iter_56600/ --gamma-beam 225.0 --gamma-cosmic 225.0 --f-sys 0.
1 
Post-hoc γ override: γ_beam=225  γ_cosmic=225  f_sys=0.1  eps=1.0
