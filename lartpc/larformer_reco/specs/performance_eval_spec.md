...
- **Metric C — slice-coverage efficiency (upstream diagnostic)**: fraction of true
  ...
  **≥50%** of the particle's **visible ionization**. This is **charge-based**, not
  ...
  (`trajfit/calo.py:dedup_charge`, `comb` = Y plane else mean(U,V) — the same
...
```
The eval reuses `trajfit/trajfit_io.py` only if it needs the slice→triplet map
...
Read these (all under `lartpc/larformer_reco/` unless noted) to
understand the reconstruction being evaluated:
- **`trajfit/README.md`** — the running log of the whole reco: track fitting,
...
- Code: **`run_nu_reco.py`** (production driver + the `nu_reco_shard` output
  schema in `_write_event`), **`trajfit/nu_interaction.py`** (the reco),
  **`trajfit/particle_momentum.py`** (kinematics + calo calibration),
  **`trajfit/trajfit_io.py`** (loader, slice↔triplet, mc pid/KE).
