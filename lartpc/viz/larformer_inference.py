...
(`slicer_predict_event` and `stage3_predict_event`). Used by:
  ...
  - `tools/viz/visualize_larformer_gt.py` (slicer prediction overlay, via
    its `--slicerpred-dir` mode)
  - `tools/viz/visualize_stage3_larformer_from_cached.py` (Stage-3
...
def track_id_color(tid: int, origin_type: int = -1, alpha: float = 1.0) -> str:
    ...
    Matches `tools/viz/visualize_larformer_gt.py:_track_id_color` byte-for-byte
...
# ---------------------------------------------------------------------------
# Detector outline (inlined: lartpc/viz/detector.py's class but no
...
# matching GT). Used by tools/viz/visualize_full_cascade.py.
#
