...
wconfig_bootstrap() {
    ...
    CONTAINERBIN=${CONTAINERBIN:-apptainer}
    BIND_FOLDERS=${BIND_FOLDERS:-"/cluster/tufts:/cluster/tufts"}
    ...
    if [ -z "${APPTAINER_CONTAINER}${SINGULARITY_CONTAINER}" ]; then
        ...
        case "${WCONFIG_CONTAINER_TYPE:-}" in
            ...
            pointcept)
                ...
                # Honor BIND_FOLDERS so this is portable (laptop binds
                # /home + /mnt/ddrive; the Tufts cluster binds /cluster).
                bind=${BIND_FOLDERS:-/cluster:/cluster}
                case "${bind}," in
                    *"${WORKDIR_BASE}:"*) ;;
                    *) bind="${bind},${WORKDIR_BASE}:${WORKDIR_BASE}" ;;
                esac
        ...
        if ! command -v ${CONTAINERBIN} >/dev/null 2>&1; then
        ...
        ${CONTAINERBIN} exec ${nv} --bind "${bind}" "${target}" \
