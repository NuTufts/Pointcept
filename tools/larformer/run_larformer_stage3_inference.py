...
`pointcept/models/LArFormer/inference.py`. Two input modes:
  ...
  --input-mode cached  (default — the immediate use case):
    ...
    `tools/larformer/build_stage12_cache_shard.py` and augmented with
    `entry_0/particle_class_id` by
    `tools/larformer/augment_stage12_cache_particle_class_id.py`). The config
...
(`tools/viz/visualize_larformer_gt.py`) can already render the Stage-2
...
Usage (cached mode):
    ./run_in_container.sh python tools/larformer/run_larformer_stage3_inference.py \\
