...
This is the Stage-3 sibling of `tools/viz/visualize_larformer_gt.py`. The
...
What changes from the Stage-2 GT viz:
  ...
  - The dataset is a `LArFormerStage12CacheDataset` reading per-event
    HDF5 caches produced by `tools/larformer/build_stage12_cache_event.py` /
  ...
  - Optional `--stage3pred-dir`: directory of `stage3pred_*.h5` files
    produced by `tools/larformer/run_larformer_stage3_inference.py`. When set,
    ...
    via `lartpc/viz/larformer_inference.py` so the panel
    can't drift from the on-disk schema.

Usage (GT only):

    python tools/viz/visualize_stage3_larformer_from_cached.py \\
...
# stays thin. See lartpc/viz/larformer_inference.py.
...
def main():
    ...
    ap.add_argument(
        ...
        help="Optional directory of stage3pred_*.h5 files produced by "
             "tools/larformer/run_larformer_stage3_inference.py. When set, a "
