...
Usage:
    ...
    ./run_in_container.sh python tools/viz/visualize_larformer_gt.py \\
...
def _load_slicerpred(path: str) -> "dict | None":
    ...
    `tools/larformer/run_slicer_inference.py` into a flat dict of numpy arrays +
    ...
    if coord_key not in pred:
        ...
        return go.Figure(data=[], layout=go.Layout(
            title=f"Level {level!r} not in this prediction file. "
                  ...
                  f"tools/larformer/run_slicer_inference.py with the current script "
        ...
        return go.Figure(
            ...
            layout=go.Layout(
                title="No prediction file for this event "
                      ...
                      "tools/larformer/run_slicer_inference.py first).",
                margin=dict(l=0, r=0, b=0, t=40),
            ),
        )
    ...
    if color_by == "pred_correct":
        ...
        if pred_mask_bool is None:
            traces.append(go.Scatter3d(x=[], y=[], z=[], name=(
                ...
                "re-run tools/larformer/run_slicer_inference.py with the latest script "
                "to regenerate.")))
        else:
            n_drawn = 0
...
def main():
    ...
    ap.add_argument("--slicerpred-dir", default=None,
                    help="Directory of slicerpred_<basename>.h5 files "
                         "produced by tools/larformer/run_slicer_inference.py. When "
