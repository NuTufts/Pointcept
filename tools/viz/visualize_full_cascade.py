...
`tools/larformer/run_larformer_stage3_inference.py --input-mode full-cascade`, OR the
`keypointpred_*.h5` files from `tools/larformer/run_larformer_fullcascade_inference.py`
...
`tools/viz/visualize_larformer_gt.py`; it shares their color conventions +
camera-sync pattern via `lartpc/viz/larformer_inference.py`.
