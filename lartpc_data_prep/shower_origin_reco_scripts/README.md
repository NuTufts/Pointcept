...
All scripts live in `Pointcept/lartpc_data_prep/shower_origin_reco_scripts/`.
...
- `Pointcept/lartpc_data_prep/convert_larlite_to_showerorigin_h5.py` — Step 2
- `Pointcept/lartpc_data_prep/process_dlmerged_to_hdf5_event_files.py` — Step 3
- `Pointcept/lartpc_data_prep/merge_reco_truth_showerorigin.py` — Step 4

## Prior Art

The existing truth-only shower fragment production scripts served as the pattern for this pipeline:
- `Pointcept/lartpc_data_prep/run_corsika_bnb_nu_pi0filter.sh` (example run script)
- `Pointcept/lartpc_data_prep/submit_bnbnu_pi0filter_corsika.sh` (example SLURM submission)
