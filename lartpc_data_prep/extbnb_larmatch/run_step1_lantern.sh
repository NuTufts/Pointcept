...
# Runs SSNet inference + LArMatch deploy on input ROOT file.
# Adapted from shower_origin_reco_scripts/run_step1_lantern.sh
# but simplified: we only need larmatch output, no shower fragments.
#
# Usage: source run_step1_lantern.sh <workdir_path> <adcname> <tbflag>
#
...
CMD="python3 ${LARMATCH_DIR}/deploy_larmatchme.py --config-file ${CONFIG_FILE} --supera ${baseinput} --weights ${LARMATCH_DIR}/${WEIGHT_FILE} --output ${lm_outfile} --min-score ${LM_MIN_SCORE} --adc-name wire --chstatus-name wire --device-name cpu --use-skip-limit ${TBFLAG}"
echo ${CMD}
${CMD}
