...
1. **`triplet_data/kpscores` is NOT stored** in the current merged_h5
   ...
   **computed on-the-fly** from `mckeypoints` via
   `lartpc_data_prep/keypoint_labels.py::compute_kpscores` (Gaussian σ
...
**What landed:**
* `lartpc_data_prep/keypoint_labels.py` — shared `compute_kpscores` (per-SP
...
* `pointcept/datasets/larformer.py`:
  ...
  * `larformer_collate`: `kpscores` in `optional_flat_keys` (flat per-SP,
    ...
    nested lists.
* `lartpc_data_prep/larformer_keypoint/test_phase0_dataloader.py` — acceptance
  test.
...
`python lartpc_data_prep/larformer_keypoint/test_phase0_dataloader.py`.
...
* `lartpc_data_prep/keypoint_labels.py::copy_mckeypoints_group` — shared
...
* Test: `lartpc_data_prep/larformer_keypoint/test_phase0_cache.py` — **PASS**
...
* `lartpc_data_prep/larformer_keypoint/train_phase1_overfit.py` — standalone
...
`python lartpc_data_prep/larformer_keypoint/train_phase1_overfit.py
