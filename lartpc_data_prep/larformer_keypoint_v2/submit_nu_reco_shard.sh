...
#
# Prereqs (generated files, must exist under trajfit_dev/data/):
#   range2ke_lar.npz  -> `python trajfit_dev/make_range2ke_npz.py`
#   calo_calib.npz    -> `python trajfit_dev/particle_momentum.py`  (refit on the
...
apptainer exec --bind /cluster:/cluster "${container}" bash -c "
  ...
  python3 ${RECODIR}/run_nu_reco.py \
