...
#SBATCH --job-name=lf-particle-smoke-s1cache
#SBATCH --mem=64G
...
#SBATCH --output=logs/lf-particle-smoke-s1cache.%j.%N.log
#SBATCH --error=logs/lf-particle-smoke-s1cache.%j.%N.err
...
OLD_CACHE=/cluster/tufts/wongjiradlab/larbys/data/ub_on_tufts/hdf5/larformer_cache_stage12__s1ep2_v6lantern_tau020
SAVE=${WORKDIR}/exp/_smoke_particle_s1cache
mkdir -p "${SAVE}"
...
find ${OLD_CACHE}/train -name '*.h5' 2>/dev/null | sort | grep -E 'mcc9|overlay' | head -200 > "${TRAIN_LIST}"; find ${OLD_CACHE}/train -name '*.h5' 2>/dev/null | sort | grep -vE 'mcc9|overlay' | head -200 >> "${TRAIN_LIST}"
