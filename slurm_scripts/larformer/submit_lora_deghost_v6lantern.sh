#!/bin/bash
# LoRA deghoster on the v6-noghosts encoder + LANTERN data — DATA-ISOLATION cell of the
# encoder-swap A/B (see lorafinetune-sonata-v6noghosts-lantern-deghost.py). Trains on
...
#SBATCH --job-name=lora-deghost-v6lantern
#SBATCH --mem=128G
...
#SBATCH --output=logs/lora-deghost-v6lantern.%j.%N.log
#SBATCH --error=logs/lora-deghost-v6lantern.%j.%N.err

set -u
WORKDIR=/cluster/tufts/wongjiradlabnu/twongj01/pointcept_env/kpv2_pointcept
CONFIG=${WORKDIR}/configs/lartpc/larformer/stage1_deghost/lorafinetune-sonata-v6noghosts-lantern-deghost.py
container=/cluster/tufts/wongjiradlabnu/larbys/containers/pointcept_cuml.sif
SAVE=${WORKDIR}/sonata/lora_deghost_v6noghosts_lantern
