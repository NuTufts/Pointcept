...
#   sbatch submit_larformer_particle_rebal_v2_warm_a100.sh

#SBATCH --job-name=lf-particle-rebalw
#SBATCH --mem=192G
...
#SBATCH --output=logs/lf-particle-rebalw.%j.%N.log
#SBATCH --error=logs/lf-particle-rebalw.%j.%N.err

set -u
WORKDIR=/cluster/tufts/wongjiradlabnu/twongj01/pointcept_env/kpv2_pointcept
CONFIG=${WORKDIR}/configs/lartpc/larformer/stage3_particle/larformer-particle-s1cache-m2frecipe-rebal-v2-warmstart.py
container=/cluster/tufts/wongjiradlabnu/larbys/containers/pointcept_cuml.sif
SCRIPT_DIR=${WORKDIR}/slurm_scripts/larformer
SELF=${SCRIPT_DIR}/submit_larformer_particle_rebal_v2_warm_a100.sh

SAVE=${WORKDIR}/exp/larformer_particle_s1cache_m2frecipe_rebal_v2_warm
CHECKPOINT=${SAVE}/model/model_last.pth
FINAL_CKPT=${SAVE}/model/epoch_4.pth
