...
#SBATCH --output=logs/larformer.v1.cascaded_ptv3hybrid_crosslevel.%j.%N.log
#SBATCH --error=logs/larformer.v1.cascaded_ptv3hybrid_crosslevel.%j.%N.err

set -u

# set the location of your copy of the repo here
WORKDIR=/cluster/tufts/wongjiradlabnu/twongj01/pointcept_env/pointcept/
CONFIG=${WORKDIR}/configs/lartpc/larformer-slicer-v1-cascaded-ptv3hybrid-crosslevel.py
...
CHECKPOINT="${WORKDIR}/exp/larformer_slicer_v1_cascaded_ptv3hybrid_crosslevel_nonzeroinit_maskdn_noamp/model/model_last.pth"
apptainer exec --nv --bind /cluster:/cluster $container bash -c "cd ${WORKDIR} && \
  cd /cluster/tufts/wongjiradlabnu/twongj01/pointcept_env/ && \
  source setenv_pointcept_only.sh && \
  cd /cluster/tufts/wongjiradlabnu/twongj01/pointcept_env/pointcept && \
